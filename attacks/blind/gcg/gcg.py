...
class GCGConfig:
    ...
    base_folder_location: str = "./attacks/blind/"
    delimiter: str = " | "
    noise_samples: int = 25000
...
class GCG:
    ...
    def setup(self,
        ...
        for noise_sample, msg, old_edit, new_edit in tqdm(zip(noise_prompts, messages, old_edits, new_edits), desc="Preparing prompts"):
            if "{optim_str}" not in noise_sample:
                noise_sample = noise_sample + "{optim_str}"

            current_messages_formatted = [{"role": "user", "content": noise_sample}]
            ...
            target = msg + self.config.delimiter + old_edit + self.config.delimiter + new_edit
    ...
    def run(
        ...
        self.setup(noise_prompts, messages, old_edits, new_edits, tokenizer, compute_device)

        optim_ids = torch.tensor(config.init, device=compute_device, dtype=torch.long).unsqueeze(0)

        # Data.
        best_prefix = optim_ids.clone()
        ...
        for step in tqdm(iterator):
            # Included a 1024 sample cap to prevent OOM, but this can be removed if you have sufficient compute
            sampled_indices = random.sample(range(len(self.history_before_ids_list)), min(1024, len(self.history_before_ids_list)))
        ...
        best_prefix = best_prefix.squeeze(0)
        self.reset()

        return GCGResult(
            ranking_loss=best_ranking_loss,
            best_tensor=best_prefix.squeeze(0),
...
def run(
    ...
    assert len(messages) == len(old_edits) == len(new_edits) == len(noise_prompts), "Length of all input lists must be the same."
    gcg = GCG(model, tokenizer, config)
    result = gcg.run(noise_prompts, messages, old_edits, new_edits, top_k=top_k)
